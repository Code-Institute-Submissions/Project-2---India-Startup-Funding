queue()
        ...
        function makeGraphs(error, startupcsv) {
         ...
         // Bar Chart1 - Funding Received by State
            var state_dim = ndx.dimension(dc.pluck('state'));
            var funding_received_by_state = state_dim.group().reduceSum(function(d) {if (isNaN(d.amount)) {
                    return 0;
                }
               return d.amount; });
            ...
            var funding_received_by_industry = industry_dim.group().reduceSum(function(d) {if (isNaN(d.amount)) {
                    return 0;
                }
               return d.amount; });
            ...
            var startup_with_most_funding = startupName_dim.group().reduceSum(function(d) {if (isNaN(d.amount)) {
                    return 0;
                }
               return d.amount; });
            ...
            // Bar Chart  - Investment Value By Month
            ...
            // var investment_value = date_dim.group().reduceSum(function(d) {
            //     if (isNaN(d.amount)) {
            //         return 0;
            //     }
            //   return d.amount;
